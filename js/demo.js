function initMap() {
	...
	trafficLayer.createRequest = function(bounds) {
		...
		var url = 'http://adriana-4.cartodb.com/api/v2/sql?format=GeoJSON&q=SELECT the_geom FROM tfl_lines WHERE the_geom %26%26 ST_SetSRID (ST_MakeBox2D(ST_Point('+bounds[0]+','+bounds[1]+'), ST_Point('+bounds[2]+','+ bounds[3]+')),4326)';
